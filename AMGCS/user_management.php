...
<form id="addEditForm" novalidate> <!-- Disable browser validation, we use JS/Server -->
        ...
        <span class="error-message" id="confirmPasswordError"></span>
            ...
            <!-- Add/Remove other roles as needed -->
        </select>
...
<script>
    ...
    $(document).ready(function() {
        ...
        if ($userTableTbody.find('tr').length > 0 && $userTableTbody.find('td').length > 1) {
             $('#userTable').DataTable({
    ...
    function closeModal(modalId) {
        ...
        if (modalId === 'addEditModal') {
             ...
             clearFormErrors('addEditForm'); // Clear any previous validation errors
              ...
              // Ensure confirm password is hidden and its value is cleared
              $('#confirmPassword').hide().prev('label').hide();
              $('#confirmPassword').val('');
        }
        if (modalId === 'deleteConfirmModal') {
            $('#deleteUserId').val('');
            $('#deleteConfirmText').text('Are you sure you want to delete this user?');
        }
    }
    ...
    function openAddModal() {
        ...
        $('#password').val('');
        $('#confirmPassword').val('');
        $('#role').val(''); // Clear the select dropdown value to its default (or first option)

        $('#password').prop('required', true); // Password IS required for add
        $('#confirmPassword').show().prev('label').show(); // Show confirm password for add
    ...
    // Removed the status parameter from the function signature
    function openEditModal(id, username, email, role) {
        ...
        $('#confirmPassword').hide().prev('label').hide(); // Hide confirm password for edit
    ...
    function submitUserForm() {
        ...
        function markError(fieldId, message) {
            ...
            if ($field.length) {
                ...
                isValid = false;
                 ...
                 // Log a warning if a server error tries to mark a non-existent field
                 console.warn("Server error attempting to mark non-existent field:", fieldId);
            }
        }
        ...
        if (!emailVal) {
         ...
         // Password (only required for 'add' or if changed during 'edit')
         ...
         if (action === 'add') {
             ...
             // Confirm password validation only applies if the field is visible (i.e., for add)
         ...
         } else if (action === 'edit' && passwordVal) { // If password is provided during edit
                ...
                if (response.success) {
                    ...
                    if (response.errors) {
                        ...
                        $.each(response.errors, function(key, message) {
                            // Assumes error keys match input names (e.g., 'username', 'email')
                         ...
                         // Use keys from response.errors to check existing fields
                         for (const fieldKey in response.errors) {
                            if ($('#' + fieldKey).length) { // Check if element exists in the current form
                                if (!serverFirstErrorField) {
                                    serverFirstErrorField = $('#' + fieldKey);
                                }
                                break; // Found the first one
                            }
                         }
                         if (serverFirstErrorField) {
                            serverFirstErrorField.focus();
                         }
