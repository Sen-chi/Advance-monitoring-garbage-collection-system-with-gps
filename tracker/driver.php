            ...
            <?php if (is_array($driverData) && !empty($driverData)): ?>
                <?php foreach ($driverData as $driver): ?>
                    <?php
                        ...
                        // --- CHANGED HERE ---
                        $contactNumber = htmlspecialchars($driver['contact_number'] ?? '');
                        ...
                        <td class='action-buttons'>
                            <?php if (!empty($driverId) && is_numeric($driverId)): ?>
                                <button class='edit-btn' onclick='openEditDriverModal(
                                    ...
                                    // --- AND HERE ---
                                    <?php echo json_encode($driver['contact_number'] ?? ''); ?>, 
...
<script>
    ...
    function populateUserDropdown(currentUserId = null) {
        ...
        if(currentUserId && !currentUserInList){
            // Failsafe handled by backend model
        }
    }
    ...
    function openEditDriverModal(id, firstName, middleName, lastName, contactNumber, currentTruckId, currentUserId, status) {
        $('#addEditForm')[0].reset();
        clearFormErrors('addEditForm');
