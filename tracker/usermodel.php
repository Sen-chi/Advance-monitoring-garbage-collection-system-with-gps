...
{
    // Ensure required keys exist, though handle_user_actions should ensure this
    if (!isset($userData['username'], $userData['email'], $userData['password'], $userData['role'], $userData['status'])) {
        // Log error or handle missing data internally if needed
        error_log("addUser: Missing required data in userData array.");
        return false;
    }
 ...
 * @param array $userData Associative array containing data to update. Can include 'username', 'email', 'password' (hashed, optional), 'role'. Status is handled separately.
...
function updateUser(PDO $pdo, int $userId, array $userData): bool
{
    ...
    // Define allowed fields that can be updated by this function
    // Status is handled by updateUserStatus now, so remove it from here.
    $allowedFields = ['username', 'email', 'password', 'role'];

    foreach ($allowedFields as $field) {
        // Check if the field is provided in the data AND is allowed
        if (array_key_exists($field, $userData)) { // Use array_key_exists to handle null/empty string explicitly if needed
             // Basic check for empty password, don't add to update if empty
             // This allows leaving password blank in the edit form to keep the old one.
    ...
    if (empty($setParts)) {
        // No fields to update provided in $userData array
        return false;
    }
    ...
    foreach ($params as $key => &$value) {
        // Determine PDO type - simple heuristic
